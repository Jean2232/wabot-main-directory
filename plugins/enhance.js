...
let handler = async (m) => {
  ...
  if (!mime) throw 'Use com fotos!'
  if (!/image\/(jpe?g|png)/.test(mime)) throw `Mime ${mime} Sem suporte!`
  ...
  await conn.sendFile(m.chat, await res.buffer(), 'hd.jpg', 'Aqui, hd certo?', m)
}
handler.help = ['hd (menção|legenda)']
handler.tags = ['tools']
